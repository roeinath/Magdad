...
def get_cleaning_day(sunday: datetime.date, day: int, day_schedule: DaySchedule):
    """
    ...
    cleaning_duties = [
        Task.new_task(datetime.datetime.combine(date, t[0]), datetime.datetime.combine(date, t[1]), t[2]) for t in day_schedule.hours
    ]

    return CleaningDay(cleaning_duties=cleaning_duties, date=date)
